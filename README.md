...
**Profile switch time:** Exclusive-host disconnects the machine you leave (no wake / dual-link battery). Firmware then **directed-then-open** advertises to the selected host (research: faster discovery), then dense undirected boost. Windows is still often slower than Mac; Device Manager → Bluetooth adapter → Power Management → uncheck “Allow the computer to turn off this device to save power.” If switches regress, set `CONFIG_TOTEM_DIR_THEN_OPEN=n`.
